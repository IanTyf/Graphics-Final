...
addEventListenersToCanvas = function(canvas) {
   ...
   let toX = x => 2 * (x-10 - r.left) / canvas.width - 1,
   ...
   canvas.addEventListener('mousemove', function(e) {
      ...
      this._response(toX(e.clientX + window.pageXOffset), toY(e.clientY + window.pageYOffset));
   });

   canvas.addEventListener('mousedown', function(e) {
      this.onPress(toX(e.clientX + window.pageXOffset), toY(e.clientY + window.pageYOffset), e.button);
      this._isDown = true ;
   });

   canvas.addEventListener('mouseup'  , function(e) {
      this.onRelease(toX(e.clientX + window.pageXOffset), toY(e.clientY + window.pageYOffset), e.button);
      this._isDown = false;
   });
   ...
   setTimeout(function() {
      ...
      canvas.setShaders = function(vertexShader, fragmentShader) {         // Add the vertex and fragment shaders:
         ...
         function addshader(type, src) {                                        // Create and attach a WebGL shader.
	      ...
	      errorMarker.innerHTML = spacer('grey', 1, 1) + '<font size=5 color=grey>\u25B6</font>';
		  ...
		  let nPixels = isFirefox ? 17 * line - 10 : 18 * line - 1;
	          errorMarker.innerHTML = spacer('grey', 1, nPixels) + '<font size=5>\u25B6</font>';
               }
...
let setColor = rgb => {
   ...
   setUniform('Matrix4fv', 'uPhong', false, [
      .6*rgb[0],.6*rgb[1],.6*rgb[2], rgb[3] ? rgb[3] : 1,
      .4*rgb[0],.4*rgb[1],.4*rgb[2],0,
...
let drawMeshInArray = i => {
   ...
   if (i == hitIndex) {
      //console.log(i);
      if (tool === 'paint') rgb = color;
      else rgb = [0.3,0.3,0.3];
   }
   if (rgb)
      setColor(rgb);
